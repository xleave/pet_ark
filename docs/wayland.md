...
第一阶段没有托盘或可视配置面板。缩放、速度、自动移动、角色、显示器和 click-through 由上述 CLI/鼠标/信号入口管理；完全 click-through 后由 `SIGUSR1` 恢复，这是当前明确保留的外部控制路径。
...
- Standalone 纯逻辑测试：70 项断言通过，覆盖状态机、移动边界、睡眠点击唤醒和动画播放器。
- Standalone 资产 validator：1 个角色、13 个 runtime 动画状态通过。
- image2 trace manifest：路径、A/B 源帧、候选帧和 accepted/rejected 字段验证通过。
- 原生 standalone：在临时开发 sysroot 中完成真实编译和链接，严格 warnings 配置下为 0 warnings。

这些结果仍不等价于 compositor 中窗口呈现和交互通过；本环境没有实际 Wayland/niri 图形会话。
...
- `SIGHUP` 的角色切换路径在加入第二个 registry 角色后实测。
